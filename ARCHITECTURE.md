...
- **Reset codes are single-use and expire in 10 minutes** - now genuinely emailed via Supabase's recovery-OTP flow (the "Reset Password" email template is configured to send `{{ .Token }}` rather than a magic link, to keep the same code-entry UX). The code is 8 digits - Supabase's own default, not the 6-digit format the old client-only prototype invented - so the input field's `maxLength` has to match Supabase's actual length, not an assumed one. Nothing is displayed on screen anymore; there is a mail server now.
